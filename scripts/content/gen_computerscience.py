...
""",
    ...
    [
        q('cs-arch-q1', 'What is the purpose of the CPU?', ["To send images to the monitor for display","To store files and programs permanently","To connect the computer to other networks","To fetch, decode and execute instructions"], "To fetch, decode and execute instructions", 'The CPU processes data and instructions by repeatedly carrying out the fetch-decode-execute cycle.', 'foundation'),
        q('cs-arch-q2', 'Which register holds the address of the next instruction to be fetched?', ["Program Counter (PC)","Accumulator (ACC)","Memory Data Register (MDR)","Memory Address Register"], "Program Counter (PC)", 'The PC holds the address of the next instruction. It is incremented during each fetch.', 'foundation'),
        q('cs-arch-q3', 'Which register stores the results of calculations carried out by the ALU?', ["Memory Address Register","Program Counter (PC)","Memory Data Register","Accumulator (ACC)"], "Accumulator (ACC)", 'The accumulator holds the results of calculations performed by the ALU.', 'intermediate'),
        q('cs-arch-q4', 'What is held in the MDR?', ["The memory address of the next instruction that is going to be fetched","Data or an instruction just fetched from, or going to, memory","Only the result of the last calculation by the ALU","The clock speed of the CPU in gigahertz"], "Data or an instruction just fetched from, or going to, memory", 'The Memory Data Register holds whatever is being transferred to or from memory.', 'intermediate'),
        ...
        q('cs-arch-q7', "A phone's CPU is upgraded from 2 to 4 cores, but one app runs no faster. What is the most likely reason?", ["Cores only affect the device's storage capacity","Adding more cores removes the CPU's cache memory","The app isn't written to use more than one core","Adding cores reduces the clock speed to zero"], "The app isn't written to use more than one core", 'Extra cores only help when a program can divide its work into tasks that run in parallel.', 'higher'),
        q('cs-arch-q8', 'Why does a larger cache usually improve CPU performance?', ["More often-used data can be kept close to the CPU, so RAM is used less","Cache is slower than RAM, so a larger cache makes the CPU wait longer","A larger cache increases the clock speed of every core in the CPU","Cache keeps files permanently when the power is switched off"], "More often-used data can be kept close to the CPU, so RAM is used less", 'Cache is much faster than RAM. The more that fits in cache, the less often the CPU has to wait for RAM.', 'higher'),
        q('cs-arch-q9', 'What is the defining feature of the von Neumann architecture?', ["The CPU has no registers of any kind","Instructions and data are kept in separate memories","Data and instructions share the same memory","It can only ever run one single program"], "Data and instructions share the same memory", 'Von Neumann machines store both program instructions and data in a single shared main memory.', 'further'),
        ...
        q('cs-arch-q11', 'Which is the best example of an embedded system?', ["The controller in a washing machine","A laptop running office software","A web server hosting many websites","A desktop PC used for playing games"], "The controller in a washing machine", 'An embedded system is a computer built into a larger device to perform one dedicated function, like controlling a wash cycle.', 'further'),
    ])
...
""",
    ...
    [
        ...
        q('cs-mem-q3', 'What does ROM typically store?', ["The bootstrap instructions that start the computer","The document or spreadsheet that the user is currently editing","Web pages that are being downloaded","Pages of virtual memory swapped out of RAM"], "The bootstrap instructions that start the computer", 'ROM is non-volatile and read-only, so it is used for the start-up instructions.', 'intermediate'),
        q('cs-mem-q4', 'Why do computers need secondary storage?', ["To take over from the CPU when it is busy","To keep data and programs when the power is off","Because RAM keeps its contents with no power","To speed up the clock of the processor"], "To keep data and programs when the power is off", 'RAM is volatile and limited in size, so non-volatile secondary storage keeps files permanently.', 'intermediate'),
        q('cs-mem-q5', 'A computer with many programs open becomes very slow. Which explanation is best?', ["The monitor resolution is set much too high","The CPU has far too much cache memory fitted","ROM is full, so programs can no longer be loaded","RAM is full, so slower virtual memory is being used"], "RAM is full, so slower virtual memory is being used", 'Swapping data between RAM and much slower secondary storage causes the slowdown.', 'higher'),
        q('cs-mem-q6', 'A photographer needs to store 8 TB of images on a desktop as cheaply as possible. Which storage is most suitable?', ["Solid state drive (SSD)","Read-only memory (ROM)","A stack of Blu-ray discs","Magnetic hard disk drive"], "Magnetic hard disk drive", 'HDDs offer very high capacity at the lowest cost per GB, and portability/durability matter less for a desktop.', 'higher'),
        q('cs-mem-q7', 'Why are solid state drives used in most modern laptops and phones?', ["They are the cheapest storage per gigabyte","They are fast, robust, light and use little power","They are volatile, so they lose their data very quickly","Their moving parts help them read data faster"], "They are fast, robust, light and use little power", 'Portable devices get knocked about and run on batteries, so SSDs are the best fit.', 'higher'),
        ...
        q('cs-mem-q10', 'Which list is in order from smallest to largest?', ["nibble, bit, byte, GB, MB, KB, TB, PB","bit, nibble, byte, KB, MB, GB, TB, PB","byte, bit, nibble, KB, GB, MB, TB, PB","bit, byte, nibble, MB, KB, TB, GB, PB"], "bit, nibble, byte, KB, MB, GB, TB, PB", 'Each step up is 1,000 (or 1,024) times bigger after the byte.', 'intermediate'),
    ])
...
""",
    ...
    [
        ...
        q('cs-dr-q5', 'Increasing the colour depth of an image will:', ["The metadata being removed from the file","More colours per pixel and a bigger file","A lower resolution for the whole image","Fewer colours available for each pixel"], "More colours per pixel and a bigger file", 'More bits per pixel means more possible colours, but every pixel now takes more storage.', 'intermediate'),
        ...
        q('cs-dr-q7', 'Adding 11001000 and 01000000 using 8 bits causes what?', ["Nothing unusual happens at all","A rounding error in the result","Overflow, as 264 needs 9 bits","A syntax error in the program"], "Overflow, as 264 needs 9 bits", '200 + 64 = 264, which is greater than 255, the largest 8-bit value.', 'higher'),
        q('cs-dr-q8', 'Why does Unicode use more bits per character than ASCII?', ["To compress text so that it takes up less storage space","To make text load more quickly on web pages and in mobile apps","To represent many more characters, including other alphabets","Because ASCII can't store any numbers or digits at all"], "To represent many more characters, including other alphabets", 'More bits give far more unique codes, so Unicode can cover every writing system.', 'higher'),
        ...
        q('cs-dr-q10', 'A 10-second clip is sampled at 44,100 Hz with a bit depth of 16. What is its size in bits?', ["7,056,000","70,560,000","441,000","705,600"], "7,056,000", '44,100 × 16 × 10 = 7,056,000 bits.', 'further'),
        q('cs-dr-q11', "Which compression should be used for a program's source code, and why?", ["Lossy — code doesn't need every single character","Lossy — it always gives the smallest file size","None — text files can never be compressed at all","Lossless — the code must be restored exactly"], "Lossless — the code must be restored exactly", 'Losing even one character could stop the program working, so only lossless is acceptable.', 'further'),
    ])
...
""",
    ...
    [
        ...
        q('cs-net-q2', 'What does DNS do?', ["Turns domain names into IP addresses","Sends and receives email between servers","Encrypts web pages before they are sent","Assigns MAC addresses to network cards"], "Turns domain names into IP addresses", 'DNS lets you type a name instead of remembering the numeric IP address.', 'foundation'),
        ...
        q('cs-net-q4', 'What is a key difference between a LAN and a WAN?', ["A LAN covers a small area on owned kit; a WAN covers a large area on leased links","A LAN always uses fibre optic cable, while a WAN uses copper cable","There is no real difference; they are two names for the same kind of network","A WAN is always wireless, while a LAN always uses physical cables to connect every device"], "A LAN covers a small area on owned kit; a WAN covers a large area on leased links", 'Ownership of infrastructure and geographical size are the two key distinctions OCR looks for.', 'intermediate'),
        q('cs-net-q5', 'Which change would NOT reduce network performance?', ["Low bandwidth on the connection","Many users streaming video at the same time","Interference on a busy wireless link","Moving from Wi-Fi to a wired Ethernet cable"], "Moving from Wi-Fi to a wired Ethernet cable", 'Wired connections are generally faster and more reliable, so this would improve performance.', 'intermediate'),
        ...
        q('cs-net-q7', 'Why are network protocols organised into layers?', ["Because the law in most countries requires networks to use them","Each layer does one job, so layers can change independently","To make networks slower but much safer for their users","So that only one protocol can ever run at any one time"], "Each layer does one job, so layers can change independently", 'Layering breaks a complex job into manageable, self-contained parts.', 'higher'),
        ...
        q('cs-net-q9', 'Which statement about MAC and IP addresses is correct?', ["Both addresses change every time a device connects to a network","A MAC address is fixed to the NIC; an IP address can change","MAC addresses are used to route data across the internet","An IP address is permanently built into the network interface card"], "A MAC address is fixed to the NIC; an IP address can change", 'MAC = fixed hardware identity; IP = logical address that depends on which network you are on.', 'further'),
        q('cs-net-q10', 'What is the role of TCP in the TCP/IP stack?', ["Encrypting web pages sent over the internet","Routing packets between different networks using IP addresses","Splitting data into packets and checking they all arrive","Translating domain names into IP addresses"], "Splitting data into packets and checking they all arrive", 'TCP handles reliable delivery; IP handles addressing and routing.', 'further'),
        q('cs-net-q11', 'What is the best reason for a school to choose client-server rather than peer-to-peer?', ["Every device keeps its own backups, so data is safer","Security, updates and backups are managed centrally","It needs no server, so it is much cheaper to set up","It has no single point of failure if a machine breaks"], "Security, updates and backups are managed centrally", 'Central management is the key advantage for an organisation with many users.', 'further'),
    ])
...
""",
    ...
    [
        q('cs-sec-q1', 'What is phishing?', ["Fake messages from a 'trusted' source that trick people into giving details","Flooding a web server with so many requests that it stops responding","A program that copies itself from one computer to another across a network","Testing a network for weaknesses before criminals can find and exploit them"], "Fake messages from a 'trusted' source that trick people into giving details", 'Phishing is a form of social engineering — it targets people, not technology.', 'foundation'),
        q('cs-sec-q2', 'What does a firewall do?', ["Monitors traffic and blocks it according to rules","Backs up all data to a remote server nightly","Encrypts every file stored on the computer's hard drive","Removes viruses from files that are infected"], "Monitors traffic and blocks it according to rules", 'A firewall filters traffic between a network and the outside world.', 'foundation'),
        ...
        q('cs-sec-q4', 'Which measure best defends against a brute-force password attack?', ["Turning off encryption so logins are processed faster","Using a larger monitor so passwords are easier to read","Locking the account after a few failed attempts","Switching from client-server to a peer-to-peer network"], "Locking the account after a few failed attempts", 'Lockouts stop an attacker from trying thousands of combinations.', 'intermediate'),
        q('cs-sec-q5', 'What is the aim of a denial of service (DoS) attack?', ["To steal passwords from the users of a website","To get physical access to a locked server room","To encrypt files and demand a payment for the key","To flood a server so it can't serve real users"], "To flood a server so it can't serve real users", 'DoS attacks make a service unavailable rather than stealing data.', 'higher'),
        q('cs-sec-q6', 'Why is encryption useful if data is intercepted?', ["It makes the data travel faster across the network","It stops the data from being intercepted at all","It removes any malware hidden inside the data","It can't be read without the decryption key"], "It can't be read without the decryption key", 'Encryption does not prevent interception — it makes the stolen data useless.', 'higher'),
        q('cs-sec-q7', 'Why do companies pay for penetration testing?', ["To install malware on competitors' computer systems","To find weaknesses by simulating attacks first","To make the company network run much faster","Because every business must do it to use the internet"], "To find weaknesses by simulating attacks first", 'Pen testers are authorised to attack the system and report what they find.', 'higher'),
        q('cs-sec-q8', 'Which best describes ransomware?', ["A physical lock fitted to the door of a server room","A program that keeps showing unwanted adverts","A type of firewall that blocks incoming traffic","Malware that encrypts files and demands payment"], "Malware that encrypts files and demands payment", 'Ransomware holds data hostage until a ransom is paid.', 'further'),
        ...
        q('cs-sec-q10', 'Why are people often called the "weakest link" in network security?', ["Passwords chosen by people are always far too long","Social engineering exploits human trust and mistakes","Most people simply cannot use computers properly","Firewalls stop working whenever a person is logged in"], "Social engineering exploits human trust and mistakes", 'The strongest firewall is useless if someone hands over their password.', 'further'),
    ])
...
""",
    ...
    [
        q('cs-sw-q1', 'Which is an example of utility software?', ["Defragmentation software","A word processor","A web browser game","A spreadsheet application"], "Defragmentation software", 'Utilities maintain or protect the computer; word processors and spreadsheets are applications.', 'foundation'),
        q('cs-sw-q2', 'What is the purpose of a device driver?', ["To let the OS communicate with a piece of hardware","To protect the computer against viruses and worms","To speed up the CPU by increasing its clock speed","To compress files so that they take up less space"], "To let the OS communicate with a piece of hardware", 'Drivers translate OS instructions into commands the device understands.', 'foundation'),
        q('cs-sw-q3', 'Which OS function allows several programs to appear to run at the same time?', ["File management and folder structure","Defragmentation of the hard disk","User management and access rights","Memory management and multitasking"], "Memory management and multitasking", 'The OS shares CPU time and allocates RAM between running programs.', 'intermediate'),
        ...
        q('cs-sw-q5', 'Why does defragmentation improve performance on a hard disk drive?', ["It deletes unused files to free up space on the disk","It increases the amount of RAM available to programs","It moves file parts together, so the head moves less","It encrypts the data so it can be read more quickly"], "It moves file parts together, so the head moves less", 'Less mechanical movement means faster reading of files.', 'higher'),
        q('cs-sw-q6', 'Why should an SSD not usually be defragmented?', ["Defragmenting an SSD deletes all of the data stored on it","Fragmentation barely slows SSDs, and extra writes wear them out","SSDs are volatile, so their data disappears when switched off","SSDs can't store files, so there is nothing there to defragment"], "Fragmentation barely slows SSDs, and extra writes wear them out", 'SSD access time is the same wherever data is stored.', 'higher'),
        q('cs-sw-q7', 'Which feature of a GUI makes it suitable for most users?', ["Windows, icons, menus and pointers are intuitive","It is designed to be used only by experts","Users must memorise many typed commands before using it","It uses less memory than any other interface"], "Windows, icons, menus and pointers are intuitive", 'GUIs are designed to be easy to pick up.', 'higher'),
        q('cs-sw-q8', 'A network technician prefers a command-line interface. What is the best reason?', ["It is much easier for beginners to learn than a graphical interface","It cannot be automated, so mistakes are less likely to happen","It uses fewer resources and allows precise, scriptable commands","It has large, colourful icons that are quick to recognise"], "It uses fewer resources and allows precise, scriptable commands", 'Experts value the speed, power and automation of typed commands.', 'further'),
        ...
        q('cs-sw-q10', 'Why is compression software useful before emailing a large set of files?', ["It encrypts the files so no one else can read them","It defragments the files so they open more quickly","It removes any viruses that the files may contain","Smaller files send faster and fit attachment limits"], "Smaller files send faster and fit attachment limits", 'Smaller files use less bandwidth and storage.', 'further'),
    ])
...
""",
    ...
    [
        ...
        q('cs-imp-q2', 'What is e-waste?', ["Unused space on a storage device","Deleted emails in the bin folder","Spam messages sent to many users","Discarded electronic devices"], "Discarded electronic devices", 'E-waste is a growing environmental problem when not recycled safely.', 'foundation'),
        ...
        q('cs-imp-q4', 'What is a key feature of open-source software?', ["Its source code can be viewed, changed and shared","It always costs a licence fee for every user","It can never be updated once it is released","Users are forbidden from sharing it with others"], "Its source code can be viewed, changed and shared", 'Open source means the source code is openly available.', 'intermediate'),
        q('cs-imp-q5', 'Copying and sharing a paid-for game online without permission breaks which law?', ["Copyright, Designs and Patents Act 1988","Computer Misuse Act 1990","Data Protection Act 2018","Freedom of Information Act 2000 (Section 1)"], "Copyright, Designs and Patents Act 1988", "Software is protected as the creator's work.", 'higher'),
        ...
        q('cs-imp-q7', 'Why might a business choose proprietary software rather than open source?', ["It comes with professional support and tested updates","It is always free to download and use in a business","It has been proven not to contain any bugs at all","Its source code can be edited freely by the business"], "It comes with professional support and tested updates", 'Businesses often value guaranteed support and accountability.', 'higher'),
        q('cs-imp-q8', "Spreading ransomware to encrypt a hospital's files is an offence under which part of the Computer Misuse Act?", ["It is not covered by the Computer Misuse Act at all","Unauthorised acts intended to impair a computer","Copyright infringement of the hospital's software","Unauthorised access to computer material, only"], "Unauthorised acts intended to impair a computer", 'Spreading malware that damages or disables systems is the third CMA offence.', 'further'),
        q('cs-imp-q9', 'Which is an environmental BENEFIT of technology?', ["E-waste being sent to landfill sites","Video calls reducing the need to travel","Energy used by large data centres","Mining rare metals for new components"], "Video calls reducing the need to travel", 'Less travel means lower carbon emissions.', 'further'),
        q('cs-imp-q10', 'What is meant by the "digital divide"?', ["The difference between binary and denary numbers","The split between local and wide area networks","The split between computer hardware and the software that runs on it","The gap between people with and without internet access"], "The gap between people with and without internet access", 'The divide can be caused by cost, location, age or skills.', 'further'),
    ])
...
""",
    ...
    [
        q('cs-alg-q1', 'What is decomposition?', ["Removing unnecessary detail from a problem","Sorting a list of data into alphabetical order","Writing a program directly in binary code","Breaking a problem into smaller parts"], "Breaking a problem into smaller parts", 'Decomposition makes a big problem manageable.', 'foundation'),
        ...
        q('cs-alg-q5', 'What is a trace table used for?', ["Encrypting data before it is transmitted","Recording variable values as an algorithm runs","Sorting lists into ascending order","Drawing flowcharts to plan an algorithm before coding it"], "Recording variable values as an algorithm runs", 'Trace tables help find logic errors and predict output.', 'intermediate'),
        ...
        q('cs-alg-q8', 'Why is merge sort usually faster than bubble sort for very large lists?', ["It never needs to compare any items to put them in order","It only works on numbers, which are quicker to compare","It splits and merges lists, so it needs far fewer comparisons","It always sorts the whole list in a single pass through it"], "It splits and merges lists, so it needs far fewer comparisons", 'Merge sort scales much better, at the cost of extra memory.', 'higher'),
        q('cs-alg-q9', 'A linear search is used on an unsorted list of 1,000 names. What is the maximum number of comparisons?', ["1,001","10","1","1,000"], "1,000", 'In the worst case the item is last (or missing), so every item is checked.', 'further'),
        q('cs-alg-q10', 'A sorted list has 1,024 items. Roughly how many comparisons does a binary search need at most?', ["About 10 or 11","About 100","About 512","About 1,024 or more"], "About 10 or 11", '1,024 halves to 1 in 10 steps (2¹⁰ = 1,024), so around 10–11 comparisons.', 'further'),
        q('cs-alg-q11', 'Which sort builds a sorted section one item at a time, placing each new item in its correct position?', ["Linear search sort","Merge sort","Insertion sort","Bubble sort"], "Insertion sort", 'Insertion sort grows a sorted part at the front of the list.', 'further'),
    ])
...
""",
    ...
    [
        ...
        q('cs-prog-q8', 'Which SQL statement returns the names of all students in Year 10?', ["SELECT Name FROM Students WHERE Year = 10","SELECT Students FROM Name WHERE 10","GET Name IN Students IF Year = 10 ORDER BY Name","FIND Name WHERE Students = 10"], "SELECT Name FROM Students WHERE Year = 10", 'SELECT field FROM table WHERE condition.', 'higher'),
        ...
        q('cs-prog-q12', 'What is the best reason to use a while loop instead of a for loop?', ["The number of repetitions is known before the loop starts running","While loops always run faster than for loops in every language","For loops cannot use variables, so a while loop has to be used","It must repeat until a condition is met, number of times unknown"], "It must repeat until a condition is met, number of times unknown", 'While is condition-controlled; for is count-controlled.', 'further'),
    ])
...
""",
    ...
    [
        ...
        q('cs-rob-q2', 'What is the purpose of a comment in code?', ["To encrypt the code so no one else can read it","To explain the code to others; it is ignored at run time","To validate the data that the user types in","To make the program run faster by telling it which lines to skip"], "To explain the code to others; it is ignored at run time", 'Comments improve maintainability.', 'foundation'),
        ...
        q('cs-rob-q4', 'Which validation check makes sure a field has not been left blank?', ["Presence check","Length and format check","Type check","Range check"], "Presence check", 'A presence check rejects empty input.', 'intermediate'),
        ...
        q('cs-rob-q6', 'For a program accepting ages 11–18, "sixteen" is which kind of test data?', ["Erroneous","Normal","Boundary","Invalid (out of range)"], "Erroneous", 'It is the wrong data type (text, not a number), so it is erroneous.', 'higher'),
        ...
        q('cs-rob-q8', 'Why is iterative testing carried out during development rather than only at the end?', ["It removes the need for any final testing of the program","It is a legal requirement for all professional software developers","It makes the finished program take up less storage space","Errors are found in each module while they are easy to locate"], "Errors are found in each module while they are easy to locate", 'Catching problems early is cheaper and easier.', 'further'),
        q('cs-rob-q9', 'Which of these is authentication rather than validation?', ["Checking that a field has not been left empty","Checking that an email address contains an @ symbol","Checking a username and password to confirm identity","Checking that a number is between 1 and 10"], "Checking a username and password to confirm identity", 'Authentication is about who the user is.', 'further'),
        q('cs-rob-q10', 'Which change most improves maintainability?', ["Putting all of the code on one line to keep it compact","Well-named sub-programs, comments and consistent indentation","Removing every comment from the code to save space","Using single-letter variable names throughout the program"], "Well-named sub-programs, comments and consistent indentation", 'Clear structure and naming make code easier to understand and change.', 'further'),
    ])
...
""",
    ...
    [
        ...
        q('cs-lang-q2', 'What does a translator do?', ["Stores programs in RAM while they run","Encrypts programs before distribution","Checks that user input is valid","Turns source code into machine code"], "Turns source code into machine code", 'Compilers, interpreters and assemblers are all translators.', 'foundation'),
        ...
        q('cs-lang-q6', 'Why is an interpreter useful while a program is being developed?', ["It stops at the first error and shows the line","It translates machine code back into Python code","It hides the source code from anyone using the program","It produces a fast, stand-alone executable file"], "It stops at the first error and shows the line", 'Line-by-line translation makes debugging quicker.', 'higher'),
        q('cs-lang-q7', 'Why might a programmer choose assembly language over a high-level language?', ["The same code runs on any type of processor","Direct hardware control and fast, efficient code","It is much easier to learn than Python or Java","It doesn't need any kind of translator to run"], "Direct hardware control and fast, efficient code", 'Useful for embedded systems and drivers where efficiency matters.', 'higher'),
        q('cs-lang-q8', 'Which statement about compiled programs is correct?', ["They cannot contain any errors once they have been compiled","The executable runs without the translator, usually faster","They always run more slowly than interpreted programs","They must be translated again every time they are run"], "The executable runs without the translator, usually faster", 'Translation happens once, up front.', 'further'),
        q('cs-lang-q9', 'What is a breakpoint in an IDE?', ["A syntax error that stops the program from being translated","A type of loop that repeats until a condition is met","The final line of the program, where execution ends","A point where the program pauses so variables can be checked"], "A point where the program pauses so variables can be checked", 'Breakpoints are a key debugging tool.', 'further'),
